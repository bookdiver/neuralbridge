...
from ..stochastic_processes.abstract_processes.auxiliary_process import AuxiliaryProcess
...
class BackwardODE:
    ...
    def solve_step(self, solver_state: namedtuple, t: float, dt: float) -> namedtuple:
        ...
        M_diag = kernel_r3(lambda t, x: L @ self.g(t) @ self.g(t).T @ L.T, t, solver_state.M_diag, dt)
    ...
    def solve(self, reverse_ts: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
...
def kernel_r3(func: callable, t: float, x: jnp.ndarray, dt: float, **kwargs) -> jnp.ndarray:
    k1 = func(t, x, **kwargs)
